...
import { api, LeadProcessingResult } from '../services/api';

const LeadUpload: React.FC<{ onUploadComplete?: () => void }> = ({ onUploadComplete }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const uploadResult = await api.uploadLeadsCSV(file);
  ...
  return (
    <div className="card">
      ...
      <h2 className="card-title">Subir Archivo CSV de Leads</h2>
      ...
      {result && (
        <div className="success-message">
          ...
          <div className="result-grid">
            <div><strong>Total Leads:</strong> {result.totalLeads}</div>
            <div><strong>Matcheados:</strong> {result.matchedCount}</div>
            <div><strong>Sin Match:</strong> {result.unmatchedCount}</div>
            <div><strong>Descartados:</strong> {result.discardedCount}</div>
